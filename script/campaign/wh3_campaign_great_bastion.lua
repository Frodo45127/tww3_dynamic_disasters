			...
			if new_threat < self.max_threat_value and not invasion_active then
				...
				if new_threat + threat_increase >= self.max_threat_value then
					...
					for i = 1, (army_vars.army_amount - kurgan_warband:military_force_list():num_items()) do
						...
						out("Frodo: Bastion Spawn X " ..tostring(position[1]) .. ".");
						out("Frodo: Bastion Spawn Y " ..tostring(position[2]) .. ".");
...
function Bastion:spawn_army(army_size, invasion_name, position_coords)
	...
	-- FRODO: Increased from 5 to 15 the radious to fix spawning errors.
	local x, y = cm:find_valid_spawn_location_for_character_from_position(self.invasion_faction, position_coords[1], position_coords[2], false, 15)
