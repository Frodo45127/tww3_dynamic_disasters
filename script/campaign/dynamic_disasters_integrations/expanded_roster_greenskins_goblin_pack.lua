...
if dynamic_disasters then
    ...
    -- Infantry
    error_message = dynamic_disasters:add_unit_to_army_template("greenskins", "lategame_goblins", "grn_inf_forest_bow", 2); if is_string(error_message) then out("\t\t" .. error_message); end
    error_message = dynamic_disasters:add_unit_to_army_template("greenskins", "lategame_goblins", "grn_inf_night_goblin_spears", 2); if is_string(error_message) then out("\t\t" .. error_message); end
