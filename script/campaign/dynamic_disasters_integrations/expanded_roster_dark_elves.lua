...
if dynamic_disasters then
    ...
    -- Ranged Infantry
    error_message = dynamic_disasters:add_unit_to_army_template("dark_elves", "midgame", "hunters_anath_raema", 2); if is_string(error_message) then out("\t\t" .. error_message); end
    error_message = dynamic_disasters:add_unit_to_army_template("dark_elves", "earlygame", "hunters_anath_raema", 3); if is_string(error_message) then out("\t\t" .. error_message); end
end
